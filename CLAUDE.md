...
- **Dice tool**: genuinely decoupled, in three layers. (1) `src/lib/dice/roll.ts` — a pure, DB-free `rollDice(faces, quantity)`. (2) `<DiceRoller>` (`src/components/dice/dice-roller.tsx`) — the actual embeddable widget: presets (d4/d6/d8/d10/d12/d20/d100), free faces/quantity/modifier inputs, a "Tirar" button, and its own result display; it knows nothing about sessions or sheets — it just computes the roll and hands the result to an `onRoll(result)` callback, optionally rendering an `extraControls` slot for whatever the embedder needs (e.g. a privacy checkbox). (3) Two independent embeddings compose it: `<SessionDiceRoller>` wires `onRoll` to the `logDiceRoll` server action (persists to `dice_rolls`, table below) plus a master-only private checkbox; `<StatDicePopover>` — one instance per `stats`-group number field on the sheet form, rendered as a small 🎲 button that opens `<DiceRoller>` in a popover anchored to that field and applies the result straight into it on roll (client-side only, no DB write — ability scores stay roll-or-type-by-hand). A later phase may swap that popover for a full-screen dice animation without touching how the result gets applied.
  - `dice_rolls` table: `faces`, `quantity`, `modifier` (-1000..1000, added to the sum for `total`), `results` (jsonb array). Only rollable by a session's participants while that session is `active`; rolls are logged against the session. Public by default; only the session's master (a participant with a readied `master`-type sheet — same `is_session_master` check used elsewhere) can mark their own roll private, enforced by RLS `WITH CHECK`, not just hidden in the UI. Visibility (RLS): public rolls readable by any participant of that session; private rolls readable only by whoever rolled them and by the session's master.
