...
- **Aventura** (`adventures` table): belongs to one Campaña, name (required), description (optional), `max_players` (≤ campaign's limit, validated in the server action — cross-table CHECK constraints aren't a thing in Postgres). Has its own `master_user_id` — different adventures in the same campaign can have different masters. Only assignable to a user who already owns a `master`-type sheet in that campaign (consistent with how "master" is defined for Ficha visibility); only the campaign creator can create/edit/delete adventures (same rule as campaign invites), any campaign member can view them.
...
- **Ficha** (sheet; `sheets` + `sheet_field_values` tables): belongs to a Campaña (reusable across that campaign's adventures, not tied to a single one), type `character` or `master`; a user can own several sheets in the same campaign (no uniqueness constraint) — the "can't be master and player at once" simplification is enforced later, at session-join time (Sesión task), not at sheet-creation time. Character sheets get one `sheet_field_values` row per `game_field` of the campaign's game (auto-created by trigger), each independently flagged `visible_on_card` (visibility is orthogonal to the group). Master sheets have no fields of their own yet. JSON export/import only applies to `character` sheets. Visibility (RLS-enforced, not app-filtered): owner edits their own sheet in full; a "master" (a user owning any `master`-type sheet in that campaign — derived from Ficha data alone, since Aventura doesn't exist yet) reads every sheet in the campaign in full, read-only; everyone else reads only the `visible_on_card` fields of other members' sheets.
